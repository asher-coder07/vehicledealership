...
def get_part_order(order_number):
    ...
    base_query = f'''
        SELECT po.purchase_order_number, po.part_vendor_name, pv.street, pv.city, pv.state, pv.postal_code, pv.phone_number, p.part_number, p.description, p.cost, p.quantity, p.status, po.total_cost 
        FROM partorder po 
    ...
    all_parts_of_order = cursor.fetchone()
